...
const monthCoreStart = source.indexOf('function __createMonthRepeatCore');
const monthCoreEnd = source.indexOf('\n', source.indexOf('const __monthRepeatCore ='));
assert.ok(monthCoreStart >= 0 && monthCoreEnd > monthCoreStart, 'Tomato must embed its own monthly core');
vm.runInContext(source.slice(monthCoreStart, monthCoreEnd), context);
...
const monthReminder = { enabled: true, interval: 'monthly', every: 2, monthlyMode: 'date', calendarMode: 'solar', monthDays: [10, 11], startDate: '2026-09-10', times: ['09:00', '18:00'] };
assert.equal(getEndDate(monthReminder, 5), '2027-01-10', 'counts are occurrences, not months or reminder times');
assert.equal(normalizeDateKey(context.__test.getNextReminderDateTime(monthReminder, new Date('2026-09-10T19:00:00'))), '2026-09-11');
assert.equal(normalizeDateKey(context.__test.getNextReminderDateTime(monthReminder, new Date('2026-09-11T19:00:00'))), '2026-11-10');
const sameDay = context.__test.getNextReminderDateTime(monthReminder, new Date('2026-09-10T10:00:00'));
assert.equal(sameDay.getHours(), 18, 'later reminder times must remain on the same selected date');
assert.equal(getEndDate({ ...monthReminder, every: 1, startDate: '2026-01-31', monthDays: [31, -1] }, 3), '2026-03-31');
assert.equal(getEndDate({ ...monthReminder, every: 1, startDate: '2026-01-31', monthDays: [31] }, 3), '2026-05-31');
const followMonthly = { ...monthReminder, interval: 'once', repeatMode: 'followTaskRepeat', taskCompletionTime: '2026-09-10', taskRepeatRule: { enabled: true, type: 'monthly', every: 2, monthDays: [10, 11], monthlyMode: 'date', calendarMode: 'solar', anchorDate: '2026-09-10' } };
assert.equal(normalizeDateKey(getFollowPreview(followMonthly, new Date('2026-09-10T19:00:00'))), '2026-09-11');
const limitedFollowMonthly = { ...followMonthly, taskRepeatRule: { ...followMonthly.taskRepeatRule, maxOccurrences: 2 }, taskRepeatState: { occurrenceCount: 1 } };
assert.equal(normalizeDateKey(getFollowPreview(limitedFollowMonthly, new Date('2026-09-10T19:00:00'))), '2026-09-11');
assert.equal(getFollowPreview(limitedFollowMonthly, new Date('2026-09-11T19:00:00')), null, 'monthly follow preview must not advance beyond the remaining count');
context.__isReminderOccurrenceBeforeScheduleEffectiveAt = () => false;
const lastStart = source.indexOf('const __getLastDueReminderDateTime =');
const lastEnd = source.indexOf('const __getSessionNotifiedSet =', lastStart);
vm.runInContext(source.slice(lastStart, lastEnd) + '\nthis.getLastMonthly = __getLastDueReminderDateTime;', context);
assert.equal(normalizeDateKey(context.getLastMonthly(monthReminder, new Date('2026-10-15T10:00:00'))), '2026-09-11');
assert.equal(context.getLastMonthly(monthReminder, new Date('2026-09-10T08:00:00')), null);
const weekReminder = { ...monthReminder, every: 1, monthlyMode: 'weekday', monthWeek: { ordinal: -1, weekday: 5 } };
assert.equal(normalizeDateKey(context.__test.getNextReminderDateTime(weekReminder, new Date('2026-09-10T19:00:00'))), '2026-09-25');
assert.equal(getEndDate(weekReminder, 3), '2026-11-27');
assert.equal(normalizeDateKey(context.getLastMonthly(weekReminder, new Date('2026-10-31T19:00:00'))), '2026-10-30');
assert.equal(context.tmRepeatCore, undefined, 'Tomato monthly recurrence must work without Task Horizon');
assert.equal(context.tmMonthRepeat, undefined, 'Tomato must not rely on a globally loaded monthly module');
assert.doesNotMatch(source, /monthly-repeat\.js/);
